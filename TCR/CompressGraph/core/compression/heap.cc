...
{
  // u is the length of the inter 
  // Tarray Thnode Thfreq 分别代表什么意思呢？
  Theap H;
  int i;
  H.sqrtu = 2;
  // 找到 sqrt(u) 整数
  ...
  // 分配 sqrt(u) 个Tarray 结构体空间，并将初始指针赋值给 H.infreq
  H.infreq = (Tarray *)malloc(H.sqrtu * sizeof(Tarray));

  // 用factor 和 minsize 初始化 H.infreq的每一个元素（Tarray）
  for (i = 1; i < H.sqrtu; i++) H.infreq[i] = createArray(factor, minsize);

  // 分配 sqrt(u) 个 Thnode 结构体空间，并将初始指针赋值给 H.freq
  H.freq = (Thnode *)malloc(H.sqrtu * sizeof(Thnode));
  H.freef = 0;
  // 初始化每一个 Thnode的next指针（int），指向其连续地址空间中的下一个 Thnode
  for (i = 0; i < H.sqrtu - 1; i++) H.freq[i].next = i + 1;
  // 最后一个 Thnode 的 next 指针指向 -1
  H.freq[H.sqrtu - 1].next = -1;

  // 分配 sqrt(u) 个 Thfreq 结构体空间，并将初始指针赋值给 H.ff
  H.ff = (Thfreq *)malloc(H.sqrtu * sizeof(Thfreq));
  H.freeff = 0;
  // 初始化每一个 Thfreq 的larger指针（int），指向其连续地址空间中的下一个 Thfreq
  for (i = 0; i < H.sqrtu - 1; i++) H.ff[i].larger = i + 1;
  H.ff[H.sqrtu - 1].larger = -1;

  // smallest 和 largest 分别代表什么意思呢？
