...
void prepare(std::vector<int> &inter) {
    ...
    // Rec: Trarray 初始化一个Trarray 结构体
    Rec = createRecords(factor, minsize);
    // Heap: Theap 初始化一个Theap 结构体
    Heap = createHeap(u, &Rec, factor, minsize);
    // Hash: Thash 初始化一个Thash 结构体
    Hash = createHash(256 * 256, &Rec);
    // L: 初始化一个长度为 u 的 Tlist 结构体数组
    L = (Tlist *)malloc(u * sizeof(Tlist));
    // 初始化Rec的 Hash Heap 和 L
...
int main(int argc, char **argv) {
    ...
    // for (int i = 0; i < csr_vlist.size() && i < 150; i++) {
    //     std::cout << csr_vlist[i] << " ";
    // }
    std::vector<int> csr_elist;
    int e_cnt = read_binary2vector(pelist, csr_elist);
    std::cout << "origin v_list: " << csr_vlist.size() << " " << "origin e_list: " << csr_elist.size() << std::endl;
    ...
    std::cout << "vertex_cnt: " << csr_vlist.size() << std::endl;
    ...
    std::cout << v_cnt << " " << rule_cnt << std::endl;
    std::cout << "depth: " << getDepth(out_vlist, out_elist, v_cnt) << std::endl;
    
    std::cout << "origin_vlist: " << csr_vlist.size() << " origin_elist: " << csr_elist.size() << std::endl;
    std::cout << "compress_vlist: " << out_vlist.size() << " compress_elist: " << out_elist.size() << std::endl;
    return 0;
}
